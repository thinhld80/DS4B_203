...
```{r}
# CODE HERE
transactions_tbl %>%plot_time_series(.date_var = purchased_at, .value = revenue)
...
google_analytics_by_page_tbl %>%
  group_by(pagePath) %>%
  plot_time_series(.date_var = date, .value = sessions, .interactive = FALSE, .facet_ncol = 4)
```
...
transactions_product_page_sessions_weekly_tbl %>%
  ...
  group_by(name) %>%
  plot_time_series(.date_var = purchased_at, .value = value, .facet_ncol = 3)
```
...
# pagevisits go up as revenue goes up for a much longer time of up to 11-12 weeks(11-12 lags)
