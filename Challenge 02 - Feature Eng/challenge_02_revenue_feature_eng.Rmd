...
recipe_spec_base  %>% 
  ...
  plot_time_series_regression(
    .date_var = purchased_at,
    .formula  = revenue_trans ~ splines::ns(x =purchased_at_index.num, df = 3)
    )
```
...
```{r}
# YOUR CODE HERE
recipe_spec_1_lag <- recipe_spec_base %>%
...
```{r}
# YOUR CODE HERE
modeltime_forecast(object = model_tbl ,new_data = testing(splits), actual_data = data_prepared_tbl) %>%
...
refit_tbl %>%
  ...
  mutate(across(.value:.conf_hi, ~standardize_inv_vec(., mean = std_mean, sd = std_sd))) %>%
